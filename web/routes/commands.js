const express = require("express");
require("dotenv").config({path: '../.env'});
const router = express.Router();
const { timestamp, getDiscordAvatarUrl } = require("../libs/utils");
const db = require("../libs/database/db");
const crypto = require('crypto');

router.get("/", (req, res) => {
	res.render('commands', { username: req.session.user.username,  avatarUrl: getDiscordAvatarUrl(req.session.user.id, req.session.user.avatar), csrfToken: req.session.csrf });
});

router.get("/list", (req, res) => {
	db.query(`SELECT commands.*, 
           u1.username AS created_by_username, 
           u2.username AS updated_by_username
    FROM commands
    LEFT JOIN users u1 ON commands.created_by = u1.discord_id
    LEFT JOIN users u2 ON commands.updated_by = u2.discord_id`, (err, results, fields) => {
		if (err) {
			console.error(err);
			return res.status(500).json({ message: 'Internal server error' });
		}
		const formattedResults = results.map(command => ({
			...command,
			created_at: new Date(command.created_at * 1000).toLocaleString(),
			updated_at: new Date(command.updated_at * 1000).toLocaleString(),
		}));

		res.json({ commands: formattedResults });
		// res.render('commands', { commands: results, csrfToken: req.session.csrf });
	});
});

router.post("/add", (req, res) => {
	if (!req.session.csrf || req.session.csrf !== req.body._csrf) {
		return res.status(403).json({ message: 'Invalid CSRF token' });
	}
	db.query('SELECT COUNT(*) as total_commands FROM commands', (err, results, fields) => {
		if (err) {
			console.error(err);
			return;
		}
		let cmdID = results[0].total_commands + 1;
	
		const { name, content } = req.body;
		if (!name || !content) {
			return res.status(400).json({ message: 'Missing required fields' });
		}

		let hash = crypto.createHash('md5').update(name.toLowerCase()).digest('hex');
		let data = [
			hash,
			name,
			content,
			created_by = req.session.user.id,
			updated_by = req.session.user.id,
			created_at = timestamp(),
			update_at = timestamp(),
		];

		db.query('INSERT INTO commands (hash, name, content, created_by, updated_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)', data, (err, results, fields) => {
			if (err) {
				console.error(err);
				return res.status(500).json({ message: 'Internal server error' });
			}
			res.status(201).json({ message: 'Command created' });
		});
	});
});

router.post("/edit/:id", (req, res) => {
	if (!req.session.csrf || req.session.csrf !== req.body._csrf) {
		return res.status(403).json({ message: 'Invalid CSRF token' });
	}
	const { name, content } = req.body;
	if (!name || !content) {
		return res.status(400).json({ message: 'Missing required fields' });
	}

	let hash = crypto.createHash('md5').update(name.toLowerCase()).digest('hex');
	let data = [
		name,
		hash,
		content,
		updated_by = req.session.user.id,
		updated_at = timestamp(),
		id = req.params.id,
	];

	db.query('UPDATE commands SET name = ?, hash = ?, content = ?, updated_by = ?, updated_at = ? WHERE id = ?', data, (err, results, fields) => {
		if (err) {
			console.error(err);
			return res.status(500).json({ message: 'Internal server error' });
		}
		res.status(200).json({ message: 'Command updated' });
	});
});

router.post("/delete/:id", (req, res) => {
	if (!req.session.csrf || req.session.csrf !== req.body._csrf) {
		return res.status(403).json({ message: 'Invalid CSRF token' });
	}

	db.query('DELETE FROM commands WHERE id = ?', [req.params.id], (err, results, fields) => {
		if (err) {
			console.error(err);
			return res.status(500).json({ message: 'Internal server error' });
		}
		res.status(200).json({ message: 'Command deleted' });
	});
});

module.exports = router;