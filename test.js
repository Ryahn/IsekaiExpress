...
const avatar = 'https://cdn.discordapp.com/avatars/72884988374167552/4cbd8b81b8d3e332e4e556bc29efe78c.png';
...
const dem = {
	...
	base: path.resolve(__dirname, './src/src/media/images/furry_license.png'),
};
...
try {
	...
	context.strokeText('ainzooalgown', dem.agent_name.x, dem.agent_name.y);
	context.fillText('ainzooalgown', dem.agent_name.x, dem.agent_name.y);
	...
	const filename = `furry_licenese_111111111_${crypto.randomBytes(9).toString('base64').replace(/\//g, '_').replace(/\+/g, '-').substr(0, 12)}.png`;
	// const attachment = new MessageAttachment(await canvas.encode('png'), filename);

	// return interaction.editReply({ files: [attachment] });
	fs.writeFileSync(path.join(__dirname, './test.png'), await canvas.encode('png'));
} catch (error) {
